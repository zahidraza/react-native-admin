...
const getColWidthMap = (columns, rows, widthMultiplier = 1) => {
  const colWidthList = columns.map((column) => {
    ...
    return { dataKey: column.dataKey, width: Math.max(Math.round(charLength * widthMultiplier * (column.charWidth || charWidth)), 50) };
  }, {});
  const totalWidth = colWidthList.reduce((acc, c) => acc + c.width, 0);
  const screenWidth = Dimensions.get('window').width - 32;

  if (totalWidth < screenWidth) {
    return colWidthList.reduce((acc, col) => ({ ...acc, [col.dataKey]: (col.width * screenWidth) / totalWidth }), {});
  } else {
